...
- `data-ratio` attribute printing the image ratio
...
.lazysrcset-ratio:after {
    content: '';
    display: block;
    height: 0;
    width: 100%;
    padding-bottom: calc(75%); /* 4/3 = default */
}
...
```html
<style>figure.lazysrcset-ratio[data-ratio="56.25"]{padding-bottom:56.25%;}</style><figure data-ratio="56.25" class="lazysrcset-ratio"><!--- image tag with srcset --></figure>
```
...
| ratio | `lazysrcset-ratio` | bool or string. additional class for class param. adds `data-ratio` attribute to figure |
